...
class CrudController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($this->fields as $f => $v) {
            if (isset($request->$f)) {
                ...
                $this->data['obj']->$f = '';
                foreach ($v as $part) {
                    $this->data['obj']->$f .= !empty($request->$part) ? $request->$part . ' ' : '';
                }
            }
        }
        $this->data['obj']->save();

        Session::flash('flash_message', __('crud.' . $this->url . '_' . ($id ? 'update' : 'add')));
    ...
    public function delete($id)
    {
        ...
        Session::flash('flash_message', __('crud.' . $this->url . '_delete'));
        return redirect($this->url);
    }
}
